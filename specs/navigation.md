...
Four tabs, each its own native-stack so detail screens push within the tab:
...
### Modal group (root level)

Screens reached from more than one tab are mounted on the root stack as modals so they slide over the current tab:

- **SourceSheet (18)** — the signature interaction. Opened from `RecordDetail`, `AnswerDetail`, and any source pill. Present as a half-height sheet (`presentation: 'modal'` now; upgrade to a true detents sheet with `formSheet`/a sheet lib). Params: `{ recordId, line? }`.
- **Reports hub + reports (23-26)** — reached from Home and Records.
- **PaymentSheet (27)** — placeholder for the native Apple Pay / Google Pay sheet; triggered before a paid report generates. Params: `{ tier }`.
...
- Modal/sheet screens (`SourceSheet`, `AnswerDetail`, `ProfileSwitcher`, reports) use the **[Sheet](../src/components/Sheet.tsx)** close button or `VoiceQuery`'s `✕` cancel instead of a back link.
