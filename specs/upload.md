...
A **drag-and-drop dropper** sits at the top: a dashed drop zone (cloud-upload medallion, "Tap to upload a file") with a row of document chips below it (e.g. Lab_Report.pdf, MRI_Scan.jpg, Prescription.pdf).
- **Tap to upload** is the primary path: tapping the drop zone opens the upload (→ Upload progress / the native file picker when wired), and tapping any chip uploads that file directly.
- **Drag** is the alternative: dragging a chip into the zone highlights it (teal fill + border, the medallion lifts) and on release starts the upload of that file (→ Upload progress with the dropped file name). The page scroll is suspended while a chip is being dragged. The chips are sample documents (the in-app stand-in for desktop drag-and-drop).

Below the dropper, a large gradient **Open Camera** hero tile (white camera medallion + watermark + a "START SCAN" pill → Camera scanner) for camera-first scanning, then a centered lock reassurance line ("Arrives encrypted · the share sheet lands here too"). The earlier secondary tiles (Photo Library / PDF or document) and the "Type manually" row were removed; the tappable drop zone now covers file/photo upload. The iOS and Android **share-sheet inbound** path also lands here (the app registers as a share target). Camera, library, and document picker are **native sheets** (see [designsystem.md](designsystem.md#native-surfaces-use-the-os-do-not-rebuild)).
